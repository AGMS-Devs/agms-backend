...
public class FileAttachmentBusinessRules : BaseBusinessRules
{
    ...
    public async Task FileTypeShouldBeValid(string fileName, string contentType)
    {
        if (contentType != "application/pdf")
            await throwBusinessException(FileAttachmentsBusinessMessages.InvalidFileType);
    }
}